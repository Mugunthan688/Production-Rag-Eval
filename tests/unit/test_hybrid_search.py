...
def test_reciprocal_rank_fusion():
    ...
    c3 = ChunkORM(id="c3", paper_id="p3", chunk_index=0, text="t3", chunking_strategy="fixed")

    dense = [(c1, 0.9), (c2, 0.8)]
    sparse = [(c2, 10.0), (c3, 8.0), (c1, 5.0)]

    fused = reciprocal_rank_fusion(dense, sparse, top_k=2)

    assert len(fused) == 2
    # c2 ranked #2 in dense, #1 in sparse -> higher fused RRF score than c1 (#1 in dense, #3 in sparse)
